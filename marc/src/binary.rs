...
const END_OF_FIELD: u8 = b'\x1E';
const END_OF_RECORD: u8 = b'\x1D';
...
const SUBFIELD_SEPARATOR: u8 = b'\x1F';
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            match self.file.read(&mut buf) {
                ...
                // Treat failure to read the file as fatal.
                Err(e) => panic!("Error reading file: {:?} {}", self.file, e),
            }
        }
...
impl DirectoryEntry {
    ...
    pub fn new(which: usize, data_start_idx: usize, dir_bytes: &[u8]) -> Result<Self, String> {
        ...
        let field_start_idx = util::bytes_to_usize(&entry[7..12])?;
...
impl Record {
    ...
    pub fn from_binary(bytes: &Vec<u8>) -> Result<Record, String> {
        ...
        let rec_size = util::bytes_to_usize(&size_bytes)?;
        ...
        if leader_bytes.len() != record::LEADER_LEN {
        ...
        let data_start_idx = util::bytes_to_usize(data_offset_bytes)?;
    ...
    fn process_directory_entry(
        ...
        let mut field = Field::new(dir_entry.tag.clone());
        field.set_ind1(field_bytes[..1][0]);
        field.set_ind2(field_bytes[1..2][0]);

        // Split the remainder on the subfield separator and
        // build Suibfield's from them.
        let mut first = true;
        for subfield in field_bytes.split(|b| b == &SUBFIELD_SEPARATOR) {
            if first {
                // Skip the first chunk since its all the stuff
                // (indicators, etc.) that comes before the first separator.
                first = false;
                continue;
            }
            ...
            let mut sf = Subfield::new(subfield[0], &[]);

            if subfield.len() > 1 {
                sf.set_content(&subfield[1..]);
            }
    ...
    pub fn to_binary(&self) -> Result<Vec<u8>, String> {
        ...
        bytes.append(&mut self.leader().value().to_vec());

        // Directory
        let num_dirs = self.build_directory(&mut bytes);

        // End-of-field after Directory
        bytes.append(&mut vec![END_OF_FIELD]);
    ...
    fn build_directory(&self, bytes: &mut Vec<u8>) -> usize {
        ...
        for field in self.control_fields() {
            ...
            // TODO bypass some of this stringifying?
    ...
    fn add_data_fields(&self, bytes: &mut Vec<u8>) {
        ...
        for field in self.control_fields() {
            bytes.append(&mut field.content().to_vec());
            bytes.append(&mut vec![END_OF_FIELD]);
        }

        for field in self.fields() {
            bytes.push(field.ind1());
            bytes.push(field.ind2());

            for sf in field.subfields() {
                bytes.push(SUBFIELD_SEPARATOR);
                bytes.push(sf.code());
                bytes.append(&mut sf.content().to_vec());
            }
