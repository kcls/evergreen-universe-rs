    ...
    fn read_next(&mut self, context: &mut XmlParseContext) -> Option<Record> {
        loop {
            ...
            if context.record_complete {
                let r = std::mem::replace(&mut context.record, Record::default());
                return Some(r);
            } else if context.doc_complete {
                // If we had a doc in progress, discard it.
                context.record = Record::default();
                ...
                if context.in_leader {
                    ...
                    if let Some(field) = record.fields_mut().last_mut() {
                        if let Some(subfield) = field.subfields_mut().last_mut() {
                            subfield.set_content(characters.as_bytes());
                        }
                    }
        ...
        for field in self.fields() {
            ...
            for sf in field.subfields() {
                ...
                xml += &format!(
                    ...
                    &escape_xml(&sf.code_char().to_string()),
                    &escape_xml(&sf.content_string()?)
                );
            }
