...
fn to_xml_string_benchmark(c: &mut Criterion) {
    let record = Record::from_breaker(r#"=LDR 01703cas a2200433 a 4500
...
=876 \\$022740217130006421$a23740217120006421$j1$zga$3no.1-45 (inc.)$d2021-07-13 12:24:26$p32101036897518$t1$yrare"#).unwrap();
    c.bench_function("to_xml_string", |b| {
        b.iter(|| {
            assert!(record.to_xml_string().contains("The APHA letter"));
        });
    });
}

criterion_group!(benches, to_xml_string_benchmark);
