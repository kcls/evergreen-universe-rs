...
fn apply_street_dates(
    scripter: &mut script::Runner,
    records: &[StreetDateRecord],
) -> Result<(), String> {
    println!("\nApplying street dates...");

    scripter.editor_mut().xact_begin()?;

    let mut defs = scripter
        .editor_mut()
        .search("acqliad", eg::hash! {"code": "street_date"})?;
    let street_date_def = defs
        .pop()
        .ok_or("street_date definition not found in acq.lineitem_attr_definition")?;
    ...
    for record in records {
        ...
        let mut invoices = scripter
            .editor_mut()
            .search("acqinv", eg::hash! {"inv_ident": inv_ident})?;
        ...
        let entries = scripter
            .editor_mut()
            .search("acqie", eg::hash! {"invoice": invoice.id()?})?;

        for entry in entries {
            let attrs = scripter.editor_mut().search(
                "acqlia",
                eg::hash! {"order_ident": "t", "lineitem": entry["lineitem"].int()?},
            )?;
            ...
            if let Some(li_id) = matched_lineitem_id {
                ...
                let existing_street_dates = scripter.editor_mut().search(
                    "acqlia",
                    eg::hash! {
                        "lineitem": li_id,
                ...
                if let Some(mut existing_attr) = existing_street_dates.into_iter().next() {
                    ...
                    if current_value != new_value {
                        println!(
                            "Updating street date for lineitem {}: {} -> {}",
