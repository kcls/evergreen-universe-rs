...
pub fn get_class(classname: &str) -> Result<&Arc<Class>, String> {
    parser()
        .classes
        .get(classname)
...
impl Parser {
    ...
    pub fn load_file(filename: &str) -> Result<(), String> {
        ...
        if GLOBAL_IDL.set(p).is_err() {
            return Err("Cannot initialize IDL more than once".to_string());
        }
    ...
    fn parse_string(xml: &str) -> Result<Parser, String> {
