...
/// assert!(dt.is_err());
///
/// let res = date::parse_datetime("2023-02-03T12:23:19-0400");
/// assert!(res.is_ok());
///
/// let d = res.unwrap().to_rfc3339();
/// assert_eq!(d, "2023-02-03T12:23:19-04:00");
///
/// let res = date::parse_datetime("2023-02-03T123");
/// assert!(res.is_err());
/// ```
pub fn parse_datetime(dt: &str) -> Result<EgDate, String> {
    if dt.len() > 10 {
...
/// ```
pub fn add_interval(date: EgDate, interval: &str) -> Result<EgDate, String> {
...
pub fn subtract_interval(date: EgDate, interval: &str) -> Result<EgDate, String> {
    let seconds = interval_to_seconds(interval)?;
    let duration = Duration::try_seconds(seconds)
        .ok_or_else(|| format!("Invalid duration seconds: {seconds}"))?;
