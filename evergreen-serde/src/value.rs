//! Wrapper class for serde_json::Value's which may also contain IDL-blessed values,
...
/// Extract the stored String and replace it with an empty string.
///
...
impl EgValue {
    ...
    /// ```
    pub fn wrapped_classname(obj: &serde_json::Value) -> Option<&str> {
        if let Some(obj) = obj.as_object() {
            if obj.contains_key(JSON_PAYLOAD_KEY) {
                return obj[JSON_CLASS_KEY].as_str();
            }
        }
    ...
    pub fn remove_class_wrapper(mut obj: serde_json::Value) -> Option<(String, serde_json::Value)> {
        EgValue::wrapped_classname(&obj)
            .map(|cname| cname.to_string())
            .map(|cname| (cname, obj[JSON_PAYLOAD_KEY].take()))
    }

    /// Translates a serde_json::Value into an EgValue treating values which
    /// appear to be IDL-classed values as vanilla serde_json::Value::Object's.
    ///
    ...
    pub fn from_json_value_plain(mut v: serde_json::Value) -> EgValue {
        match v {
            serde_json::Value::Null => EgValue::Null,
            serde_json::Value::Bool(b) => EgValue::Boolean(b),
            serde_json::Value::String(_) => EgValue::String(take_string(&mut v).unwrap()),
            serde_json::Value::Number(n) => EgValue::Number(n),
            ...
            serde_json::Value::Object(mut map) => {
                ...
                for key in keys.drain(..) {
                    ...
                    hash.insert(key, EgValue::from_json_value_plain(val));
                }
    ...
    pub fn from_json_value(mut v: serde_json::Value) -> Result<EgValue, String> {
        ...
        if !is_blessed {
            for key in keys.drain(..) {
                let val = map.remove(&key).unwrap();
                let val = EgValue::from_json_value(val)?;
                hash.insert(key, val);
            }
        ...
        for field in idl_class.fields().values() {
            ...
            // No need to store NULL entries since blessed values
            // have a known set of fields.

            if !val.is_null() {
                hash.insert(field.name().to_string(), EgValue::from_json_value(val)?);
            }
        }
    ...
    pub fn create(classname: &str, mut v: EgValue) -> Result<EgValue, String> {
        v.bless(classname)?;
        Ok(v)
    }
...
/*
    ...
    /// Same as serde_json::Value::is_empty()
    ///
    /// # Examples
    ///
    /// ```
...
impl TryFrom<serde_json::Value> for EgValue {
    type Error = String;
    fn try_from(v: serde_json::Value) -> Result<EgValue, String> {
...
impl TryFrom<(&str, EgValue)> for EgValue {
    type Error = String;
    fn try_from(parts: (&str, EgValue)) -> Result<EgValue, String> {
