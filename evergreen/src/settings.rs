...
impl SettingContext {
    ...
    pub fn org_id_json(&self) -> json::Value {
        self.org_id.map(json::from_str).unwrap_or(JSON_NULL)
    }
    pub fn user_id_json(&self) -> json::Value {
        self.user_id.map(json::from_str).unwrap_or(JSON_NULL)
    }
    pub fn workstation_id_json(&self) -> json::Value {
        self.workstation_id.map(json::from_str).unwrap_or(JSON_NULL)
    }
