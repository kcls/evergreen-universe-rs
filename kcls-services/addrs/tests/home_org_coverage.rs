    ...
    fn test_location(&mut self, location: &Location) -> EgResult<i64> {
        ...
        if result.is_object() && !result["home_ou"].is_null() {
            if let Some(org_id) = result["home_ou"].as_i64() {
                ...
                Err(format!(
                    "Invalid home_ou value for {} (lat: {}, lon: {}): {:?}",
                    location.name, location.lat, location.lon, result["home_ou"]
                )
                .into())
            }
        } else if result.is_object() && result["home_ou"].is_null() {
            Err(format!(
                "No home_ou assigned for {} (lat: {}, lon: {})",
                location.name, location.lat, location.lon
            )
            .into())
        } else {
            Err(format!(
                "Invalid response format for {} (lat: {}, lon: {}): {:?}",
                location.name, location.lat, location.lon, result
            )
            .into())
        }
    }
...
fn load_test_data() -> TestData {
    ...
    let json_content = fs::read_to_string(json_path).expect("Failed to read test data JSON file");

    serde_json::from_str(&json_content).expect("Failed to parse test data JSON")
}
...
fn test_home_org_coverage() {
    ...
    if !failures.is_empty() {
        println!(
            "\nLocations without org unit assignments ({}):",
            failures.len()
        );
...
fn test_verify_expected_org_units() {
    ...
    let locations_with_expected: Vec<_> = all_locations
        .iter()
        .filter(|loc| loc.expected_org.is_some())
        .collect();

    println!(
        "\nVerifying {} locations with expected org units:",
        locations_with_expected.len()
    );
