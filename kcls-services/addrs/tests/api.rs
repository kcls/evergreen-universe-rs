...
impl AddrsApiTester {
    ...
    fn call_api(&mut self, method: &str, params: Vec<EgValue>) -> EgResult<Vec<EgValue>> {
    ...
    fn call_api_one(&mut self, method: &str, params: Vec<EgValue>) -> EgResult<Option<EgValue>> {
...
fn test_home_org_lookup() {
    ...
    assert!(
        response["home_ou"].is_number() || response["home_ou"].is_null(),
        "home_ou should be a number or null"
    );
    assert!(
        response["is_reciprocal"].is_boolean(),
        "is_reciprocal should be a boolean"
    );
}
...
fn test_address_autocomplete() {
    ...
    search.insert(
        "search".to_string(),
        EgValue::from("1 Microsoft Way Redmond"),
    );
...
fn test_invalid_session_token() {
    ...
    if let Ok(Some(response)) = result {
        ...
        assert!(response.is_object(), "Response should be an object");
        assert!(
            response.has_key("home_ou"),
            "Response should contain home_ou"
        );
        assert!(
            response.has_key("is_reciprocal"),
